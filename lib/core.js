...
function runner(src, dst, action) {
  var context = {
    ...
    wrap: function(count) {
      var target = this._getTarget();
      var from = target.actions.length - count;
      if (from < 0 || from >= target.actions.length) throw new RangeError("Wrap count out of range");
      target._selection = {
        from: from,
        to: target.actions.length - 1
      };
      return this;
    },
    wrapAll: function() {
  ...
  var fn = function(cb, stream, opts) {
    ...
    if (!stream && context.src) {
      ...
      stream = proxy.createStream(context.src, context.srcOpts);
    }
    ...
    context.actions.forEach(function (action, i) {
      ...
      runWrapper(context._wrapperMap.begin, i, function(wrapper) {
        var ws = wrapper.begin.call(context, stream, wrapper.beginOpts);
        if (ws != stream) {
      ...
      if (action.toggled) {
        if (action.hasContext) {
          stream = action(cb, stream, {noWrite: true});
        } else {
      ...
      runWrapper(context._wrapperMap.end, i, function(wrapper) {
        var ws = wrapper.end.call(context, stream, wrapper.endOpts);
        if (ws != stream) {
          stream = stream.pipe(ws);
        }
      });
    ...
    if (!opts.noWrite && _.isString(context.dst)) {
      // console.log("Dst: " + context.dst + ", " + JSON.stringify(context.dstOpts));
      stream = stream.pipe(proxy.writeStream(context.dst, context.dstOpts));
    }
